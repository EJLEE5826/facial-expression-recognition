...
def train_model(dataloaders, model, criterion, optimizer, scheduler, num_epochs=25):
    ...
    for epoch in range(num_epochs):
        ...
        if epoch != 0 and epoch % 1 == 0:
            # checkpoint model
            torch.save(model,
                       os.path.join(args.model_folder, f"model_{str(epoch)}.pth"))
...
def main(args):
    ...
    train_ds = FERPlusDataset(args.base_folder, mode="train", transforms=train_transform)
    val_ds = FERPlusDataset(args.base_folder, mode="val", transforms=val_transform)
    test_ds = FERPlusDataset(args.base_folder, mode="test", transforms=test_transform)
